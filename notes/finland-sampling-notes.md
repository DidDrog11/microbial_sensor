...
- Heikki asked for GPX covering 44–48 and 54–60. Photos of his logbook taken as the fallback for animal-to-unit pairs; to be kept in `data-raw/wp1_finland/henttonen/` (ignored). Peatland grids and any forest SQ with no voles will be dropped from the host analysis; their soils still map the environment.
- On precision: the 4 m median offset between his fixes and ours is corner repeatability (device + observer position + the corner being a fuzzy place: traps 1–2 m off the marker, soil at the nearest runway), not device error alone. Protocol §2 reworded.
